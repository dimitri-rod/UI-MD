...
This reads: inside "Navbar," the button labeled "Get Started."

**Rules**

1. **Named containers** (plain-text container names, e.g. `Navbar`, `Hero`, `Pricing Section`) are referenced by that name.
2. **Elements** (buttons, links, checkboxes, radios, headings) are referenced by their symbol + label, e.g. `[Get Started]`, `[Contact Sales](contact)`, `<x> Monthly`.
...
Instruction: *solid button, background Accent 1*
```
