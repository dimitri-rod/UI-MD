...
2. **Shape is Function:** brackets define primitives — `[[Label]]` a button, `[Label](url)` a link, `[x]`/`[ ]` a checkbox (add a `*single-select*` rule for radio-style behavior), `<Label>` a tag.
